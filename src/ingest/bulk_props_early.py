"""Bulk-pull EARLY (kickoff - 72h) player-prop odds for player_receptions
and player_rush_attempts, 2024-2025 regular season only, and parse into a
long-format parquet.

Same structure as bulk_props.py (which pulls the CLOSING snapshot, all six
markets, 2023-2025 including postseason) — this is a narrower companion
pull: an early-window snapshot for the two markets used in
week_calibration.py / line_skew.py, to see whether the same lines move
between kickoff-72h and kickoff-10min.

Snapshot per game: commence_time - 72 hours. Every event's raw JSON is
cached to data/raw/props_early/{event_id}.json before parsing — an event
already cached is never refetched, so this script is safe to interrupt
and resume.
...
RAW_PROPS_DIR = REPO_ROOT / "data" / "raw" / "props_early"
EVENT_INDEX_PATH = RAW_PROPS_DIR / "_event_index.parquet"
OUTPUT_PATH = REPO_ROOT / "data" / "interim" / "props_early.parquet"

SEASONS = [2024, 2025]
WEEKS = range(1, 19)  # regular season only, no postseason

MARKETS = "player_receptions,player_rush_attempts"
N_MARKETS = MARKETS.count(",") + 1
REGION = "us"
N_REGIONS = 1
ODDS_FORMAT = "decimal"

SNAPSHOT_BEFORE_KICKOFF = pd.Timedelta(hours=72)

LOG_EVERY_N = 25
CREDIT_FLOOR = 5_000

COST_PER_EVENT = 10 * N_MARKETS * N_REGIONS  # Odds API historical odds pricing: 10 credits/market/region
...
def build_game_list() -> pd.DataFrame:
    """One row per game, 2024-2025 regular season, with UTC commence_time
    and the early snapshot (commence_time - 72h) as an ISO8601 string."""
    ...
    games["snapshot_ts"] = games["commence_time"] - SNAPSHOT_BEFORE_KICKOFF
...
def discover_event_ids(games: pd.DataFrame, force_refresh: bool = False) -> pd.DataFrame:
    ...
    Note: this queries the events list AT THE 72h-EARLY SNAPSHOT anchor
    (not kickoff-1h like bulk_props.py), since we need the event to
    already be listed 72h out — if the book hasn't posted an event that
    early, discovery here will legitimately fail to match it, distinct
    from a props-fetch error."""
    ...
    for (season, week), group in games.groupby(["season", "week"]):
        anchor = (group["commence_time"].min() - SNAPSHOT_BEFORE_KICKOFF - pd.Timedelta(hours=1)).strftime(
            "%Y-%m-%dT%H:%M:%SZ")
...
def run(index: pd.DataFrame, rosters: pd.DataFrame, limit: int = None) -> pd.DataFrame:
    ...
    for i, row in enumerate(to_process.itertuples(), 1):
        ...
        pool = build_roster_pool(rosters, row.season, row.week, row.home_team, row.away_team, game_type="REG")
...
def report_projected_cost(index: pd.DataFrame) -> None:
    n_matched = int(index.event_id.notna().sum())
    n_cached_already = sum(1 for eid in index.event_id.dropna() if _cache_path(eid).exists())
    n_to_fetch = n_matched - n_cached_already
    projected = n_to_fetch * COST_PER_EVENT
    print(f"\nProjected cost: {n_matched} matched events, {n_cached_already} already cached, "
          f"{n_to_fetch} to fetch x {COST_PER_EVENT} credits/event = {projected} credits")


def main(limit: int = None) -> pd.DataFrame:
    games = build_game_list()
    print(f"Game list: {len(games)} games, {games.season.nunique()} seasons (regular season only)")
